...
'''
i = 0
cam = False          # 카메라 프로그램이 켜집니다. 즉 실사용시엔 False가 될 예정
...
def main() :
    global frame, frame_height, detected_emotion, CEF_COUNTER, TOTAL_BLINKS, blink_animation, cap
    if __name__ == "__main__":         # main 함수
        with mp_face_mesh.FaceMesh(
                ...
                utils.colorBackgroundText(
                    frame, f'emotion detected : {detected_emotion}', FONTS, 0.7, (30, 240), 2)

                # 여러 얼굴을 감지하도록 되어있지만 구현 구조상 1로 고정한다.
                if results.multi_face_landmarks:
                    for face_landmarks in results.multi_face_landmarks:
                        # True면 감지된 모든 랜드마크를 바로바로 보여줍니다.
                        mesh_coords = landmarksDetection(frame, results, False)
                        eye_ratio = blinkRatio(
                    ...
                    # else함수에 있는 내용은 깜빡인 횟수를 카운팅하는 내용, 눈의 애니메이션을 넣는 활동서 사용
                        ...
                        mouth_ratio = mouthRatio(
                            frame, mesh_coords, MOUTH, test=False)
                    ...
                    # 만약 가상 캐릭터와 매칭을 시킨다면 아래내용은 제외
                        ...
                        if cv2.waitKey(2) & 0xFF == 27:  # esc가 눌렸을 경우 종료
                            break
    vid_lbl.after(0, main)  # run again after 1000ms (1s)
...
mainFrame = customtkinter.CTkFrame(master=mainWindow)
...
def stop_vid():
    global cam
    cam = False         
    img_label = customtkinter.CTkLabel(mainFrame, image = off_photo)
    img_label.place(x = 25, y = 50)
    if cap:
        cap.release()
        cv2.destroyAllWindows()


vid_lbl = customtkinter.CTkLabel(mainFrame, text = " 11111")
vid_lbl.place(x = 0, y = 0)

on_photo=PhotoImage(file='test.png')
off_photo=PhotoImage(file='test2.png')

def start_vid_clicked():
    img_label = customtkinter.CTkLabel(mainFrame, image = on_photo)
    img_label.place(x = 25, y = 50)
    TurnCameraOn.after(1000, start_vid)

    
#Buttons
TurnCameraOn = customtkinter.CTkButton(mainWindow, command=start_vid_clicked, text = "시작", height = 180, width = 300)
TurnCameraOn.pack(pady=12, padx=10)
TurnCameraOn.place(x=85.6, y=310)
TurnCameraOff = customtkinter.CTkButton(mainWindow, command=stop_vid, text = "정지", height = 180, width = 300)
TurnCameraOff.pack(pady=12, padx=10)
TurnCameraOff.place(x=414.2, y=310)

button_3 = customtkinter.CTkButton(mainWindow, text = "버튼3", height = 100, width = 200)
button_3.pack(pady=12, padx=10)
button_3.place(x=514.2, y=50)
